...
- PUT
  ...
  '이거로 덮어 써' 정도의 의미. PATCH와 함께 데이터 수정과 연관된 메서드이다. 단 수정이라기보다는 데이터 덮어쓰기에 가깝다. 자칫하다간 멀쩡한 데이터가 날아갈 수 있으므로 PATCH를 사용하는 것이 좋다.

  

- PATCH

  '다른 건 손대지 말고 방금 보낸 것만 넣어서 업로드해 줘'. PUT과 함께 데이터 수정과 연관된 메서드. 원래의 데이터에 손을 대지 않기 때문에 PUT에 비해서 조금 더 안전하다. 
